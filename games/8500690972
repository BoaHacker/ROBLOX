...
Main:AddToggle('Autofarm', function(state)
	...
	while true do
		...
		wait()
		local nextStage = workspace.Spawns:FindFirstChild('Checkpoint '.. require(game.ReplicatedFirst.Framework.Framework):GetModule('Client Data Manager'):GetData().Stage + 1)
		if nextStage then
			player.Character.HumanoidRootPart.CFrame = nextStage.CFrame * CFrame.new(0, 5, 0)
		end
	end
end)
