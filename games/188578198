...
Main:AddToggle('Autofarm', function(state)
	...
	while true do
		...
		wait()
		player.lapevent:FireServer(tick())
	end
end)

Main:AddButton('Collect Wheelhunt', function()
	for i, v in pairs(workspace.wheelhunt:GetChildren()) do
		v.CFrame = player.Character.HumanoidRootPart.CFrame
	end
end)

Teleports:AddButton('Lobby', function()
	local folder = workspace.lobby['spawn plates']:GetChildren()
	local spawn = folder[math.random(1, #folder)]
	player.Character.HumanoidRootPart.CFrame = spawn.CFrame * CFrame.new(0, 5, 0)
end)

Teleports:AddButton('I Sleep', function()
	player.Character.HumanoidRootPart.CFrame = workspace.lobby['i sleep Wall'].CFrame * CFrame.new(-5, 0, 0)
end)

Teleports:AddButton('Grand Opening!', function()
	player.Character.HumanoidRootPart.CFrame = workspace['mod']['floor'].CFrame * CFrame.new(0, 5, 0)
end)
