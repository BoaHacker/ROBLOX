...
Main:AddToggle('Autofarm', function(state)
	...
	while true do
		...
		wait()
		for i, v in pairs(workspace.Players[player.Slot.Value].Trains:GetChildren()) do
			v:Destroy()
		end
		for i, v in pairs(workspace.Players[player.Slot.Value].Barricades:GetChildren()) do
			v:Destroy()
		end
		for i, v in pairs(workspace.Players[player.Slot.Value].Coins:GetDescendants()) do
			if v.Name == 'Coin' then
				game.ReplicatedStorage.Events.Client.CoinTouch:FireServer(v)
			end
			if v.Name == 'Key' then
				game.ReplicatedStorage.Events.Client.Boost:FireServer(v)
			end
			if v.Name == 'Jetpack' or v.Name == 'Jump' or v.Name == 'Magnet' then
				v:Destroy()
			end
		end
	end
