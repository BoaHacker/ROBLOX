...
esp.Overrides.GetColor = function(model)
	if game.Players[model.Name].Backpack:FindFirstChild('Ruler') or workspace[model.Name]:FindFirstChild('Ruler') then return Color3.fromRGB(255, 0, 0) end
...
Main:AddButton('Random Notebook', function()
	function teleport()
		local folder = workspace.School:GetChildren()
		local tool = folder[math.random(1, #folder)]
		if tool.Name == 'Notebook' and tool:FindFirstChild('ClickDetector') then
			player.Character.HumanoidRootPart.CFrame = tool.CFrame
		else
			teleport()
		end
	end
...
Main:AddButton('EXIT Door', function()
	player.Character.HumanoidRootPart.CFrame = workspace.School.ExitDoor.CFrame
end)
