...
local jobType = 'Receptionist'
Main:AddToggle('Autofarm', function(state)
	getgenv().Autofarm = state
	if not getgenv().Autofarm then game.ReplicatedStorage.Knit.Services.JobService.RE.OnEndedJob:FireServer() return end
	game.ReplicatedStorage.Knit.Services.JobService.RE.OnStartedJob:FireServer(jobType)
	while true do
		if not getgenv().Autofarm then return end
		wait()
		game.ReplicatedStorage.Knit.Services.JobService.RE.OnOrderCompleted:FireServer(true)
		workspace.Jobs[jobType].Customers:ClearAllChildren()
	end
end)

Main:AddLabel('Job Type:')
Main:AddDropdown({'Receptionist', 'Cashier', 'Vet'}, function(state)
	if getgenv().Autofarm then game.ReplicatedStorage.Knit.Services.JobService.RE.OnStartedJob:FireServer(state) end
	jobType = state
end)
...
Settings:AddButton('Destroy UI', function()
	ui.gui:Destroy()
	antiAFK = false
	if getgenv().Autofarm then game.ReplicatedStorage.Knit.Services.JobService.RE.OnEndedJob:FireServer() getgenv().Autofarm = false end
