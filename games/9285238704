...
Main:AddToggle('Auto Clicker', function(state)
	getgenv().AutoClicker = state
	while true do
		if not getgenv().AutoClicker then return end
		wait()
		if player.PlayerGui.UI_Main.UI.TopMenu.ClickHelper.Visible == true then
			game.VirtualUser:ClickButton1(Vector2.new(mouse.X, mouse.Y))
		end
	end
end)

Main:AddButton('Win', function()
	player.Character.HumanoidRootPart.CFrame = workspace.Environment.Stage1K.Sign.CFrame
end)
...
Player:AddButton('Reset to Default', function()
	player.Character.Humanoid.WalkSpeed = player.leaderstats['🏃TopSpeed'].Value
