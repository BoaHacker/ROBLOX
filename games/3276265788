...
local coolDown = 0.1
Pets:AddToggle('Auto Desert', function(state)
	getgenv().AutoDesert = state
	while true do
		if not getgenv().AutoDesert then return end
		wait(coolDown)
		game.ReplicatedStorage.rEvents.openCrystalRemote:InvokeServer('openCrystal', 'Desert Crystal')
	end
end)

Pets:AddToggle('Auto Electro', function(state)
	getgenv().AutoElectro = state
	while true do
		if not getgenv().AutoElectro then return end
		wait(coolDown)
...
Settings:AddButton('Destroy UI', function()
	...
	getgenv().AutoDesert = false
	getgenv().AutoElectro = false
