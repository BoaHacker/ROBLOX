...
Main:AddButton('Skip Room', function()
	if player.leaderstats.Mode.Value == 'Daily' and player.leaderstats.Room.Value ~= 'End' then
		local maxDistance = math.huge
		local nearestButton
		for i, v in pairs(workspace.DailyModeRooms:GetDescendants()) do
			if v.Name == 'ClickDetector' then
				local distance = (v.Parent.Position - player.Character.HumanoidRootPart.Position).magnitude
		...
		if nearestButton then
			player.Character.HumanoidRootPart.CFrame = CFrame.new(nearestButton.Parent.Position)
			wait(0.1)
			fireclickdetector(nearestButton)
		end
	else
		local button = workspace.Modes[player.leaderstats.Mode.Value.. 'Mode']['Room'.. player.leaderstats.Room.Value]['Door/Button'].Button
		player.Character.HumanoidRootPart.CFrame = CFrame.new(button.Position)
		wait(0.1)
