...
Main:AddToggle('Kill Aura', function(state)
	...
	while true do
		...
		for i, v in pairs(workspace.Mobs:GetChildren()) do
			if v:FindFirstChild('Enemy') and v:FindFirstChild('HumanoidRootPart') then
				local magnitude = (v.HumanoidRootPart.Position - player.Character:WaitForChild('HumanoidRootPart').Position).magnitude
				if magnitude < 50 then
					game.ReplicatedStorage.GameRemotes.DamageEvent:FireServer(v.HumanoidRootPart, v.Enemy)
				end
			end
		end
	end
