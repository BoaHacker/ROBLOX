...
Main:AddToggle('Autofarm', function(state)
	...
	while true do
		...
		wait()
		local magnitude = (workspace.Winner.Portal.WinPart.Position - player.Character.HumanoidRootPart.Position).magnitude
		if magnitude > 135 then
			player.Character.HumanoidRootPart.CFrame = workspace.Winner.Portal.WinPart.CFrame
		end
	end
end)

Main:AddButton('Get Secret Badge', function()
	game.ReplicatedStorage.sendRickRollbadge:FireServer()
end)

local coolDown = 0.1
Crates:AddToggle('Auto Common', function(state)
	getgenv().AutoCommon = state
	while true do
		if not getgenv().AutoCommon then return end
		wait(coolDown)
		game.ReplicatedStorage.crate:FireServer('buy', 'common')
	end
end)

Crates:AddToggle('Auto Rare', function(state)
	getgenv().AutoRare = state
	while true do
		if not getgenv().AutoRare then return end
		wait(coolDown)
		game.ReplicatedStorage.crate:FireServer('buy', 'rare')
	end
end)

Crates:AddToggle('Auto Legendary', function(state)
	getgenv().AutoLegendary = state
	while true do
		if not getgenv().AutoLegendary then return end
		wait(coolDown)
		game.ReplicatedStorage.crate:FireServer('buy', 'legendary')
	end
end)

Crates:AddLabel('Wait Time:')
Crates:AddDropdown({'0.1', '0.5', '1', '2'}, function(state)
...
Settings:AddButton('Destroy UI', function()
	...
	getgenv().AutoCommon = false
	getgenv().AutoRare = false
	getgenv().AutoLegendary = false
